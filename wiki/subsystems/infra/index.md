...
| **Startup** | `src/startup/` | Non-blocking parallel startup jobs — config/env check, tools-loaded + brain-files report, model cache warm, RSI |
| **Logging** | `src/logging/` | `tracing`-based logger — env-filter, JSON output, file rotation + cleanup |
| **Utils** | `src/utils/` | Shared utilities — sanitization, approval UI, config watcher, etc. |
