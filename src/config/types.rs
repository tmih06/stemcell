...
impl Config {
    ...
    fn apply_env_overrides(mut config: Self) -> Result<Self> {
        ...
        if let Some(v) = env_bool("OPENCRABS_DEBUG_LSP") {
            config.debug.debug_lsp = v;
        }
        if let Some(v) = env_bool("OPENCRABS_PROFILING") {
            config.debug.profiling = v;
        }
        if let Some(v) = env_bool("OPENCRABS_CRABRACE_ENABLED") {
            config.crabrace.enabled = v;
        }
        if let Some(v) = env_bool("OPENCRABS_CRABRACE_AUTO_UPDATE") {
