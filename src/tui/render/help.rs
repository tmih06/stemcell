...
pub(super) fn render_help(f: &mut Frame, app: &App, area: Rect) {
    ...
    let mut left = vec![
        ...
        kv("Alt+Enter", "New line (multiline)", blue),
        kv("Escape (x2)", "Clear input / abort", blue),
        kv("Page Up/Down", "Scroll history", blue),
        kv("@", "File picker", blue),
        Line::from(""),
        section_header("INPUT EDITING"),
        kv("↑ / ↓", "Line nav / start-end / history", blue),
        kv("← / →", "Move cursor", blue),
        kv("Ctrl/Alt+←→", "Jump word", blue),
        kv("Home / End", "Start / end of line", blue),
        kv("Ctrl+W", "Delete word", blue),
        kv("Alt+Backspace", "Delete word", blue),
        kv("Ctrl+U", "Delete to start of line", blue),
        Line::from(""),
