...
pub(crate) async fn handle_message(
    ...
    // `TelegramSurface::deliver`. We resolved the session here (owner shares the
    // TUI session; guests key per-chat), so it rides along as `session_hint`.
    // Stash the per-turn delivery context (thread routing, group-record, TTS) so
    // `deliver` can reproduce it without holding the original message.
    telegram_state
        .set_delivery_context(
            msg.chat.id.0,
            crate::channels::telegram::DeliveryContext {
    ...
    if !gateway.publish_inbound(inbound) {
        tracing::warn!(
            "Telegram: gateway rejected inbound (queue full or closed) for chat {}",
            msg.chat.id.0
        );
    }
