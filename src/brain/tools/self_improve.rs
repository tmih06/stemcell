//! Self-Improve Tool
//!
//! Proposes and applies improvements to brain files based on feedback analysis.
//! Requires human approval. Changes are tracked in IMPROVEMENTS.md for auditing.
...
impl Tool for SelfImproveTool {
    ...
    fn description(&self) -> &str {
        "Apply a self-improvement based on feedback analysis. Modifies brain files \
         (SOUL.md, AGENTS.md, etc.) or records improvement decisions in IMPROVEMENTS.md. \
         Always requires human approval. Use feedback_analyze first to identify what \
         needs improvement, then use this tool to apply the change."
    }
    ...
    fn input_schema(&self) -> Value {
        serde_json::json!({
            ...
            "properties": {
                "action": {
                    "type": "string",
                    "description": "What to do: 'propose' (log to IMPROVEMENTS.md without changing brain files), 'apply' (edit a brain file + log), 'list' (show pending/applied improvements)",
                    "enum": ["propose", "apply", "list"]
                },
                "target_file": {
                    "type": "string",
                ...
                "description": {
                    "type": "string",
                    "description": "For 'propose'/'apply': human-readable description of the improvement"
                },
                "rationale": {
                    "type": "string",
                    "description": "For 'propose'/'apply': why this improvement is needed (reference feedback data)"
                },
                "content": {
                    "type": "string",
    ...
    async fn execute(&self, input: Value, context: &ToolExecutionContext) -> Result<ToolResult> {
        ...
        match action {
            "list" => {
                ...
                match std::fs::read_to_string(&improvements_path) {
                    ...
                    Err(e) => Ok(ToolResult::error(format!(
                        "Failed to read IMPROVEMENTS.md: {e}"
                    ))),
                }
            }
            ...
            "propose" => {
                ...
                if description.is_empty() {
                    return Ok(ToolResult::error(
                        "description is required for 'propose'".to_string(),
                    ));
                }
                ...
                let mut file = std::fs::OpenOptions::new()
                    ...
                    .map_err(|e| {
                        crate::brain::tools::ToolError::Execution(format!(
                            "Failed to open IMPROVEMENTS.md: {e}"
                        ))
                    })?;
                file.write_all(entry.as_bytes()).map_err(|e| {
                    crate::brain::tools::ToolError::Execution(format!(
                        "Failed to write IMPROVEMENTS.md: {e}"
                    ))
                })?;

                // Record in feedback ledger if DB available
                ...
                Ok(ToolResult::success(format!(
                    "Improvement proposed and logged to IMPROVEMENTS.md: {description}"
                )))
            }
            ...
            "apply" => {
                ...
                Ok(ToolResult::success(format!(
                    "Improvement applied to {target_file} and logged to IMPROVEMENTS.md: {description}"
                )))
            }
