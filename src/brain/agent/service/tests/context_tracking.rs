...
fn test_base_context_tokens_uses_real_tool_schemas() {
    ...
    rt.block_on(async {
        ...
        let service_no_tools = AgentService::new_for_test(provider.clone(), context.clone()).await
            .with_system_brain("Brain.".to_string());

        let registry = ToolRegistry::new();
        registry.register(Arc::new(MockTool));
        let service_with_tools = AgentService::new_for_test(provider, context).await
            .with_system_brain("Brain.".to_string())
            .with_tool_registry(Arc::new(registry));
