...
- self_improve: Propose or apply changes to your brain files (SOUL.md, TOOLS.md, etc.). ALWAYS requires human approval.
...
3. Call self_improve with action='propose' to suggest a concrete improvement
4. If the user approves, call self_improve with action='apply' to update your brain files

Do NOT call these tools every turn. Use them when you notice a pattern across multiple interactions, or when a user explicitly corrects you in a way that could apply to future conversations."#;
...
pub async fn build_feedback_digest(pool: crate::db::Pool) -> Option<String> {
    ...
    out.push_str(
        "Use feedback_analyze for deeper analysis. \
         If you see patterns, use self_improve to propose fixes.\n\n",
    );
    Some(out)
}
