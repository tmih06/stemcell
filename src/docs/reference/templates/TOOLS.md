...
| `generate_image` | `prompt` | `image`, `filename` |
...
| `parse_document` | `path` | `max_chars`, `pages`, `page_range`, `include_metadata` |
...
> - `exa_search` — EXA AI neural/semantic search. Available by default via free MCP endpoint (no key needed). Optional: set `EXA_API_KEY` in keys.toml for direct API access with higher rate limits. `search_type` can be `"auto"`, `"neural"`, `"fast"`, `"deep-lite"`, `"deep"`, `"deep-reasoning"`, or `"instant"`. Best for technical docs, API references, code examples, conceptual queries.
> - `brave_search` — **Optional.** Brave Search API with privacy focus. Available only when the user sets `[search.brave] enabled = true` in config and provides `BRAVE_API_KEY` in keys.toml. Best for current events / news / time-sensitive queries.
>
> **Web / GitHub / browser routing — pick the right surface:**
> - **Research / docs / "look up X"** → `exa_search` (if available) → `brave_search` (if available) → `web_search`. NEVER `browser_navigate` for research.
> - **Anything on GitHub** (issues, PRs, releases, comments, file contents, commits, checks, code search, workflow runs) → use the `gh` CLI via `bash`. It is preinstalled, authenticated, supports `--json` / `--jq` for structured filtering, and is far cheaper than navigating github.com in a browser. Examples: `gh pr view 123 --json title,body,comments,reviews`, `gh issue list --label bug --json number,title`, `gh release view v0.3.31`, `gh api repos/OWNER/REPO/commits/SHA/check-runs`.
> - **`browser_navigate`** is ONLY for: (a) the user explicitly asks to open / view / interact with a page; (b) the task requires clicking / typing / submitting / scrolling / running JavaScript against a live DOM; (c) genuine last resort after every search route has been exhausted. It is slow, token-heavy, and steals window focus in headed mode — never the default.
...
> **`generate_image`:** Generate an image from a text prompt using Google Gemini. Returns the saved file path. Automatically sends as a native image on all channels — just include `<<IMG:path>>` in your reply or the channel handler sends it for you. Optionally accepts an input image (local path or HTTPS URL) via the `image` parameter for img2img editing — replacing elements, restyling, compositing logos, or modifying user-uploaded images. The OpenAI-compatible backend does not support input images. Requires `[image.generation] enabled = true` in config. Run `/onboard:image` to set up.
...
> **`parse_document`:** Extract text from PDF, DOCX, HTML, JSON, XML, TXT, MD. Returns plain text. For multi-page PDFs use `page_range` (string like `"1-30"`, `"5,7,10-15"`, or `"3"`) — easier than `pages` for spans. `pages` accepts a JSON integer array like `[1,2,3,4,5]`. Both can be supplied; they merge and dedupe. Returns text with `--- Page N ---` markers when a range is requested. When an incoming PDF's inline preview was truncated, the original file is saved to `~/.opencrabs/tmp/files/` and the path appears in the preview — feed that path to `parse_document` with the next `page_range` to read on. Use `max_chars` only when you want a length-bounded preview of a known-huge document.
